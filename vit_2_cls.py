...
class VisionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        # 新增：用于使用 cls token 的轻量 ID head（用于实际 forward）
        ...
        # --- 在 VisionTransformer.__init__ 中加入 cls_token ---
    ...
    def forward_features(self, x):
        ...
        # 2. 加两个 cls token：顺序 [cls_id, cls_head, patches]
        cls_tokens = self.cls_token.expand(B, -1, -1)   # [B, 2, D]
        x = torch.cat((cls_tokens, x), dim=1)           # [B, 2+N, D]
        x = self.pos_drop(x)

        # 3. mask 只作用在 patch tokens（从下标 2 开始）
        if self.training and self.mask_ratio > 0:
            cls_part = x[:, :2, :]          # [B, 2, D]
            patch_part = x[:, 2:, :]        # [B, N, D]
            patch_part, _, _ = self.random_masking(patch_part)  # 这里 ids_restore 已经没用了
            x = torch.cat([cls_part, patch_part], dim=1)        # [B, 2 + N_masked, D]
        ...
        x = self.norm(x.float())            # [B, 2 + N', D]

        # 5. 拆出两个 cls token
        cls_all = x[:, :2, :]               # [B, 2, D]
        cls_id  = cls_all[:, 0:1, :]        # [B, 1, D]
        cls_head = cls_all[:, 1:1+1, :]     # [B, 1, D]

        # 不再返回 patch_out / patch_flat
        return cls_id, cls_head

    def forward(self, x):
        cls_id, cls_head = self.forward_features(x)   # 都是 [B,1,D]

        # ------- id branch：用第一个 cls token -------
        id_vec = cls_id.squeeze(1)                    # [B, D]
        x_id = self.feature_cls(id_vec)                   # [B, num_classes] 或 [B, D_id]
        norm_main = torch.norm(x_id, p=2, dim=1, keepdim=True)
        emb_main = x_id / (norm_main + 1e-6)

        # ------- head branch：用第二个 cls token -------
        head_vec = cls_head.squeeze(1)                # [B, D]
        x_head = self.head_feature(head_vec)          # [B, head_proj_dim]
        head_norm = torch.norm(x_head, p=2, dim=1, keepdim=True)
        head_emb = x_head / (head_norm + 1e-6)

        return emb_main, norm_main, head_emb, head_norm, emb_main
